...
raw <- read.csv("~/Desktop/Landfills/MA_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "MA"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Desktop/Landfills/MA_details.csv")
