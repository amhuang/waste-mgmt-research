...
raw <- read.csv("~/Desktop/Landfills/AR_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)

# Changes owners, adds facility name type ot
for (i in seq_len(size)) {
  if (raw[i,3]!="") {
    if (grepl("( county|city of|co.|WWTP|RSWMD|SWD)", raw[i,3], ignore.case=TRUE)) {
      raw[i,3] <- "Government"
    }
  }
  if ( !(raw[i, "Category"] %in% raw[i,"Name"]) )
  new_name <- paste(raw[i,"Name"], raw[i, "Category"])
  raw[i,"Name"] <- new_name
}
...
for (i in 141:344) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(details[i,"Name"], details[i,"City"], "AR"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Desktop/Landfills/AR_details.csv")


# GET DETAILS
details <- read.csv("~/Desktop/Landfills/AR_details.csv", stringsAsFactors = FALSE)
...
which <- which(duplicated(details[,"Names"]))
which <- which[which > 340]
details[which, ] <- ""
