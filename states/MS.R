...
raw <- read.csv("~/Desktop/Landfills/MS_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)

# REVERSE GEOCODING - Get address and county

for (i in seq_len(size)) {
  if ( grepl("[0-9]", raw[i,"Lat"]) ){
    response <- GET(url="https://maps.googleapis.com/maps/api/geocode/json",
                    query = list (
                      key = "",
                      latlng = paste0(raw[i,"Lat"], ",", raw[i,"Long"])
                      ))
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Street.Address"], raw[i,"City"], "Mississippi"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
  content <- content(response)
