...
raw <- read.csv("~/Desktop/Landfills/KS_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "Kansas"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(raw, "~/Desktop/Landfills/KS_details.csv")
