library(httr)
original <- read.csv("~/Downloads/Landfills/OK_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:95,]
size <- nrow(raw)
...
write.csv(details, "~/Downloads/Landfills/OK_details.csv")


# GET DETAILS
# hours, website

details <- read.csv("~/Downloads/Landfills/ok_details.csv", stringsAsFactors = FALSE)[2:51]
  ...
  if (!is.na(place_ids[i])) {
    response <- GET(url="https://maps.googleapis.com/maps/api/place/details/json",
                    query = list(
                      ...
                      fields = "formatted_address,website,opening_hours,formatted_phone_number"
                    ))
    content <- content(response)
