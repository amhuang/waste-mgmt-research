...
details <- read.csv("~/Downloads/Landfills/TX_details.csv", stringsAsFactors = FALSE)
raw <- original[1:754,]
size <- nrow(raw)
...
raw[dups,] <- NA
write.csv(raw, "~/Downloads/Landfills/TX_raw_clean.csv")
...
for (i in seq_len(size)) {
  if(is.na(raw[i,"Zip"])) {
    response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                    query = list (
                      key = "",
                      input = paste(raw[i,"Street.Address"], raw[i,"City"], "TX", sep=", "),
                      inputtype = "textquery",
    ...
    raw[i,"Zip"] <- gsub("[^0-9]", "", addr[3])
  }
}
write.csv(raw, "~/Downloads/Landfills/TX_details.csv")
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "TX"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
  content <- content(response)
...
write.csv(details, "~/Downloads/Landfills/TX_details.csv")


# GET DETAILS
# hours, website

details <- read.csv("~/Downloads/Landfills/TX_details.csv", stringsAsFactors = FALSE)[,2:51]
