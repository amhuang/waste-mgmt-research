library(httr)
original <- read.csv("~/Downloads/Landfills/WY_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:155,]
size <- nrow(raw)
...
write.csv(raw, "~/Downloads/Landfills/WY_raw_1.csv")
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,2], raw[i,5], "WY"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
  content <- content(response)
...
write.csv(details, "~/Downloads/Landfills/WY_details.csv")


# GET DETAILS
# hours, website

details <- read.csv("~/Downloads/Landfills/WY_details.csv", stringsAsFactors = FALSE)[2:51]
