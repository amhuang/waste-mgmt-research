...
raw <- read.csv("~/Downloads/Landfills/GA_raw.csv", stringsAsFactors = FALSE)
raw <- raw[1:506,]
size <- nrow(raw)

which(duplicated(raw[,"Street.Address"]))
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "GA"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Downloads/Landfills/GA_details.csv")


# GET DETAILS
# hours, website, phone
details <- read.csv("~/Downloads/Landfills/GA_details.csv", stringsAsFactors = FALSE)[,2:51]
