...
raw <- read.csv("~/Desktop/Landfills/CO_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)

# CLEANING UP RECYCLING FAC DATA
...
for(i in seq_along(add)) {
  zips[i] <- add[[i]][length(add[[i]])]
  cities[i] <- add[[i]][length(add[[i]])-1]
  add[[i]][length(add[[i]])] <- add[[i]][length(add[[i]])-1] <- ""
}
arcgis[,"Zip"] <- zips
arcgis[,"City"] <- cities
arcgis[,"Address"] <- gsub(",", "", unlist(lapply(add, toString)))
write.csv(arcgis, "~/Desktop/Landfills/arcgis.csv")
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "CO"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Desktop/Landfills/CO_details.csv")
