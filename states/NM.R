library(httr)
original <- read.csv("~/Downloads/Landfills/NM_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:293,]
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "NM"),
                    inputtype = "textquery",
...
write.csv(details, "~/Downloads/Landfills/NM_details.csv")


# GET DETAILS
# hours, website

details <- read.csv("~/Downloads/Landfills/NM_details.csv", stringsAsFactors = FALSE)[,2:51]
