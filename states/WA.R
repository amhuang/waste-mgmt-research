library(httr)
original <- read.csv("~/Downloads/Landfills/WA_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:681,]
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,2], raw[i,5], "WA"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Downloads/Landfills/WA_details.csv")


# GET DETAILS
# hours, website

details <- read.csv("~/Downloads/Landfills/WA_details.csv", stringsAsFactors = FALSE)[2:51]
