library(httr)
original <- read.csv("~/Downloads/Landfills/PA_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:344,1:49]
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,2], raw[i,5], "PA"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Downloads/Landfills/PA_details.csv")


# Obtain details with place IDs

details <- read.csv("~/Downloads/Landfills/PA_raw.csv", stringsAsFactors = FALSE)[,2:51]
