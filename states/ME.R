...
raw <- read.csv("~/Desktop/Landfills/ME_raw.csv", stringsAsFactors = FALSE)
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "Maine"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
  ...
  if (!is.na(place_ids[i])) {
    response <- GET(url="https://maps.googleapis.com/maps/api/place/details/json",
                    query = list(
                      ...
                      fields = "website,opening_hours,formatted_address"
                    ))
    content <- content(response)
...
write.csv(details, "~/Desktop/Landfills/ME_details.csv")
