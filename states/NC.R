...
original <- read.csv("~/Downloads/Landfills/NC_raw.csv", stringsAsFactors = FALSE)
raw <- original[1:304,]
size <- nrow(raw)
...
for (i in seq_len(size)) {
  response <- GET(url="https://maps.googleapis.com/maps/api/place/findplacefromtext/json",
                  query = list (
                    key = "",
                    input = paste(raw[i,"Name"], raw[i,"City"], "NC"),
                    inputtype = "textquery",
                    fields = "place_id")
  )
...
write.csv(details, "~/Downloads/Landfills/NC_details.csv")


# GET DETAILS
# hours, website, phone

details <- read.csv("~/Downloads/Landfills/NC_details.csv", stringsAsFactors = FALSE)[,2:51]
